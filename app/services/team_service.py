...
class TeamService:
    ...
    async def update_team(self, team_id: str, team_update: UpdateTeamSchema) -> Dict:
        ...
        if updated:
            ...
            raise HTTPException(
                status_code=404, detail="Team not found or update failed"
            )
    ...
    async def delete_team(self, team_id: str) -> bool:
        ...
        if result:
            ...
            raise HTTPException(
                status_code=404, detail="Team not found or deletion failed"
            )
    ...
    async def add_team_member(self, team_id: str, member_data: dict) -> bool:
        ...
        if result:
            ...
            raise HTTPException(
                status_code=404, detail="Team not found or member addition failed"
            )
    ...
    async def remove_team_member(self, team_id: str, member_id: str) -> bool:
        ...
        if result:
            ...
            raise HTTPException(
                status_code=404, detail="Team not found or member removal failed"
            )
    ...
    async def get_team_members(self, team_id: str) -> dict:
        ...
        Raises:
            HTTPException: If not found.
        """
        team_details = await self.team_repository.get_team_members(team_id)
        if team_details is not None:
            return team_details
        else:
            raise HTTPException(
                status_code=404, detail="Team not found or no members found"
            )
    ...
    async def update_team_member(
        self, team_id: str, member_id: str, member_data: dict
    ) -> bool:
        """
        ...
        Raises:
            HTTPException: If not found.
        """
        result = await self.team_repository.update_team_member(
            team_id, member_id, member_data
        )
        if result:
            return True
        else:
