from pydantic import BaseModel, Field, EmailStr, BeforeValidator, field_validator, ConfigDict, field_serializer
...
class TeamMemberModel(BaseModel):
    ...
    """
    member_id: Optional[PyObjectId] = Field(default_factory=lambda: ObjectId(), alias="_id")
    name: str = Field(..., min_length=1, max_length=255, json_schema_extra={"strip_whitespace": True})
    email: EmailStr
    role: Role  # Now uses Enum directly
    teams: Optional[List[PyObjectId]] = Field(default_factory=list) # Store only ObjectIds
...
class TeamModel(BaseModel):
    ...
    """
    team_id: Optional[PyObjectId] = Field(default_factory=lambda: ObjectId(), alias="_id")
...
class UpdateTeamSchema(BaseModel):
    ...
    """
    name: Optional[str] = Field(None, min_length=1, max_length=255, json_schema_extra={"strip_whitespace": True})
